...
void dothelighting(vec3 suncolor, vec3 mooncolor, vec3 blockcolor, vec3 worldcolor, float shadows) {
	...
	float shade = lmdata.g;
	if(entity.y == 1) {
		shade *= ((smoothstep(13.0, 14.0, lmcoord.g * 15.0) * shadows) + (1.0 - shadows));
	} else {
		shade *= ((smoothstep(13.5, 14.5, lmcoord.g * 15.0) * shadows) + (1.0 - shadows));
	}
	...
	if (entity.x == 101) {
		...
		color += vec4(vec3(vis * OW_WT_B), pow(vis, 2.1 - OW_WT_B));
		color *= glcolor * vec4(vec3(1.0), OW_WT_A);
	}
	if (entity.x == 102) {
		float vis = clamp(dot(color.rgb, vec3(1.0 / 3.0)) - 0.25, 0.0, 1.0);
		color += vec4(vec3(vis * NT_PT_B), pow(vis, 2.1 - NT_PT_B));
		color *= glcolor * vec4(vec3(1.0), NT_PT_A);
	}
	lmcolor = vec4((sunstr * suncolor) + (moonstr * mooncolor) + (blockstr * blockcolor) + (screenBrightness * worldcolor), 1.0);
	color.rgb *= mix((sunstr * suncolor / (1.0 + (3.0 * darknessFactor))) + (moonstr * mooncolor) + (blockstr * blockcolor / (1.0 + (4.0 * darknessFactor))) + (clamp(screenBrightness * worldcolor, 0.0, 1.0) * (1.0 - darknessFactor)), vec3(1.0 - (darknessFactor * 0.9)), nightVision);
	if (color.a < 0.1) {
		discard;
	}
